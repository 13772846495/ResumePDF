...
function useSelectTheme() {
  ...
  return (themeConfigValues: any) => {
    ...
    let initTheme = { id: 'dark', fontColor: '#ffffff', backgroundColor:'#27292c' };
...
function useReadAppConfigThemeFile() {
  return () => {
    return new Promise((resolve: (value: { [key: string]: any }) => void, reject: (value: Error) => void) => {
...
function useUpdateAppConfigThemeFile() {
  ...
  return (updateKey: string, updateValues: any, callback?: () => void) => {
    getAppPath().then((appPath: string) => {
      const jsonPath = path.join(appPath, 'appConfig/theme.config.js');
